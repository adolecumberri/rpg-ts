...
class Status {
  ...
  activate: (character: defaultCharacter) => IActivateReturn = (character) => {
    ...
    if (this.duration.type === 'PERMANENT' && this.whenToApply === 'PER_ACTION') {
      ...
      this.duration.type === 'PERMANENT' &&
      (this.whenToApply === 'ONCE' && this.hasAlreadyBeenApplied === false)
    ) {
      execute();
  ...
  resetStatus() {
    ...
    for (const key in this.originalStatus) {
      if (this.originalStatus.hasOwnProperty(key)) {
