import { AffectedStatDescriptor, StatusActivationFunction, StatusApplicationMoment, StatusDuration, StatusDurationTemporal, StatusUsageFrequency } from "../types";
    ...
    activate(character: Character) {
        ...
        if (isPermanentAndPerAction || isPermanentAndOnce || isTemporalAndValid || isUsageFrequencyValid) {
            for (const stat of this.statsAffected) {
                ...
                const result = action({
                    from: character[stat.from],
                    to: character[stat.to],
                    value: stat.value,
                });
