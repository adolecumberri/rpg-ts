...
class LogManagerClass {
    ...
    addLogStatus(status: Status, action: 'added' | 'removed') {
        ...
        solution += ` ${action === 'added' ? 'adds' : 'removes'} ${status.name.toUpperCase()} (id:${status.id}) from his Status List`
    ...
    getLogHeader() {
        let d = new Date()
        let formattedDate = `${d.getDay()}/${d.getMonth()}/${d.getFullYear()} - ${d.getMinutes() + 1}:${d.getHours() + 1}`
