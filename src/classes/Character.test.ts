...
import { checkAttack } from "../helper/errorControllers"
...
describe('Character tests', () => {
    ...
    describe('character is created correctly', () => {
        ...
        test("Created Character with correct Stats", () => {
            ...
            // custom id setted
            let charc = new Character({ id: 'carlos' })

            expect(charc.id).toBe('carlos')

            //is alive setted case.
            let deadChar = new Character({ isAlive: false })
    ...
    describe('character attacks', () => {
        const ATTACKS = {
            NORMAL: { discriminator: discriminators.ATTACK_OBJECT, type: ATTACK_TYPE.NORMAL, value: 10, status: { statusLastExecution: false, statsAffected: [], value: {} } },
            MISS: { discriminator: discriminators.ATTACK_OBJECT, type: ATTACK_TYPE.MISS, value: 0, status: { statusLastExecution: false, statsAffected: [], value: {} } },
            CRITICAL: { discriminator: discriminators.ATTACK_OBJECT, type: ATTACK_TYPE.CRITICAL, value: 2, status: { statusLastExecution: false, statsAffected: [], value: {} } },
        };
    ...
    describe('character defends', () => {
        describe('character deffence works correctly', () => {
            let finalDamage = basic_character.defend(basic_character.attack())
        ...
        describe('character evades attack', () => {
            ...
            let solution = char.defend(char.attack())
            expect(solution.type).toEqual('EVASION')
            expect(solution.value).toEqual(0)
        })

        describe('character min value is set', () => {
            let wall = new Character({ stats: { defence: 9999 } })
            let solution = wall.defend(char.attack())
            expect(solution.value).toEqual(0)
        })
        ...
        describe('defence function can be override and works', () => {
            ...
            char.setDefenceFunction(() => {
                let solution = getDefaultDefenceObject({ value: -5 })
                return solution
            })
    ...
    describe('character receives extra params', () => {
        test('character can receive and access to unexpected params', () => {
            let char_with_unexpected_params = new Character({
                stats: { mana: 100 },
                getMana: () => {
